    ...
    ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(fetchedVehicles)
        ...
        for key, vehicleData in ipairs(fetchedVehicles) do
            ...
            table.insert(menuElements, {
                ["label"] = "Utilizar " .. GetLabelText(GetDisplayNameFromVehicleModel(vehicleProps["model"])) .. " Matricula - " .. vehicleData["plate"],
                ["vehicle"] = vehicleData
            })
        end
        ...
        if #menuElements == 0 then
            table.insert(menuElements, {
                ["label"] = "No estacione ningún vehículo aquí.."
            })
...
OpenVehicleMenu = function()
    ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(fetchedVehicles)
            ...
            for _, vehicle in ipairs(gameVehicles) do
                if DoesEntityExist(vehicle) then
                    ...
                    if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
                        table.insert(menuElements, {
                            ["label"] = GetLabelText(GetDisplayNameFromVehicleModel(vehicleProps["model"])) .. " with plate - " .. vehicleData["plate"] .. " - " .. dstCheck .. " metros.",
        ...
        if #menuElements == 0 then
            table.insert(menuElements, {
                ["label"] = "No tienes vehiculos fuera."
            })
        end
...
ChooseVehicleAction = function(vehicleEntity, callback)
    ...
    local menuElements = {
        {
            ["label"] = (GetVehicleDoorLockStatus(vehicleEntity) == 1 and "Bloquear" or "Desbloquear") .. " Vehiculo.",
            ["action"] = "change_lock_state"
        },
        {
            ["label"] = "Para " .. (GetIsVehicleEngineRunning(vehicleEntity) and "apagar" or "encender") .. " el motor.",
            ["action"] = "change_engine_state"
        },
        {
            ["label"] = "Para " .. (DoesBlipExist(cachedData["blips"][vehicleEntity]) and "aoagar" or "encender") .. " rastreador GPS.",
...
VehicleAction = function(vehicleEntity, action)
    ...
    if action == "change_lock_state" then
        if dstCheck >= Config.RangeCheck then return ESX.ShowNotification("Está demasiado lejos del vehículo para comprobarlo.") end
        ...
        ESX.ShowNotification(GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicleEntity))) .. " con matricula - " .. GetVehicleNumberPlateText(vehicleEntity) .. " esta ahora " .. (vehicleLockState == 1 and "BLOQUEADO" or "DESBLOQUEADO"))
...
SpawnLocalVehicle = function(vehicleProps)
	...
	if not ESX.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		ESX.ShowNotification("Mueva el vehículo que está en el camino.")
...
SpawnVehicle = function(vehicleProps)
	...
	for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
			if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
				ESX.ShowNotification("Este vehículo está en la calle, no puedes sacar 2 de los mismos vehículos..")
	...
	ESX.Game.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
        ...
        ESX.ShowNotification("Sacas tu vehículo")
	...
	if DoesEntityExist(vehicle) then
		...
		ESX.TriggerServerCallback("garage:validateVehicle", function(valid)
			if valid then
				...
				ESX.ShowNotification("Estacionaste tu vehículo.")
			else
				ESX.ShowNotification("¿Realmente eres dueño de este vehículo?")
			end
